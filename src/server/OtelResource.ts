import { resourceFromAttributes } from "@opentelemetry/resources";
import {
  ATTR_SERVICE_NAME,
  ATTR_SERVICE_VERSION,
} from "@opentelemetry/semantic-conventions";
import { ServerEnv } from "./ServerEnv";

export function getOtelResource() {
  return resourceFromAttributes({
    [ATTR_SERVICE_NAME]: "openback",
    [ATTR_SERVICE_VERSION]: "1.0.0",
    ...getPromLabels(),
  });
}

export function getPromLabels() {
  const workerId = ServerEnv.workerId();
  return {
    "service.instance.id": ServerEnv.hostname(),
    "openback.environment": ServerEnv.env(),
    "openback.host": ServerEnv.host(),
    "openback.domain": ServerEnv.domain(),
    "openback.subdomain": ServerEnv.subdomain(),
    "openback.component":
      workerId !== undefined ? "Worker " + workerId : "Master",
  };
}
