...
void main() {
  ...
  // Preserve the aircraft's original route-box animation. Only tanks need the
  // world-space angle to keep their ring circular and symmetric.
  vec2 radial = vWorld - vTarget;
  float angle = isTank > 0.5
    ? atan(radial.y, radial.x)
    : atan(vLocal.y, vLocal.x);
