...
export class UnitDisplay extends LitElement implements Controller {
  ...
  connectedCallback() {
    ...
    // Android hides its navigation bar for a fullscreen game and slides it
    // back over the page on a swipe. window.innerHeight does not change when
    // that happens, so the bar was laid out for a screen taller than the one
    // the player could see and its last row sat under the buttons. The visual
    // viewport is the part actually visible, and it does report the change.
    window.visualViewport?.addEventListener("resize", this.refreshHudLayout);
    window.visualViewport?.addEventListener("scroll", this.refreshHudLayout);
  ...
  disconnectedCallback() {
    ...
    window.visualViewport?.removeEventListener("resize", this.refreshHudLayout);
    window.visualViewport?.removeEventListener("scroll", this.refreshHudLayout);
  ...
  private readonly refreshHudLayout = () => {
    ...
    // `||` rather than `??` on the widths: a zero is as useless as a missing
    // value here, and a detached or not-yet-laid-out element measures zero.
