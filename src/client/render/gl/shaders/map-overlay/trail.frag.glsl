...
void main() {
  ...
  if (uAltView != 0) {
    ...
    if (count <= 0) {
      ...
      // pass; the stamped centerline underneath is its first-color spine.
      color = texelFetch(uEffect, ivec2(o, rowBase), 0).rgb;
    } else if (int(texelFetch(uEffect, ivec2(o, rowBase + 1), 0).a + 0.5) == 1) {
      // transition — the whole trail cross-fades through the color list.
      ...
      // gradient — cyclic bands scroll across the world-space trail.
