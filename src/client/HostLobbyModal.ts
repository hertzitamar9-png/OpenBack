...
export class HostLobbyModal extends BaseModal {
  ...
  private updateLobbyHistory(lobbyUrl: string): void {
    ...
    const localLobbyPath =
      new URL(lobbyUrl).pathname + new URL(lobbyUrl).search;
    history.replaceState(
      null,
      "",
      lobbyIdHidden ? "/streamer-mode" : localLobbyPath,
    );
  }
  ...
  // A player who is starting a game is still nominally in the lobby, so
